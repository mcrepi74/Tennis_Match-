...
features_match = ["tourney_id", "tourney_level",
                  "surface", "tourney_date", "round", "best_of"]

features_player = ["id", "seed", "entry",
                   "hand", "ht", "age", "rank", "rank_points"]
features_player = [("p1_"+f) for f in features_player] + \
    [("p2_"+f) for f in features_player]
outcome = "p1_won"
...
keep = [r[1]["p1_id"] in ids and r[1]["p2_id"]
        in ids for r in df_data.iterrows()]
df_data = df_data.iloc[keep]
print(len(df_data))


# replacing missing entry feature with "unknown"
df_data[["p1_entry", "p2_entry"]] = df_data[[
    "p1_entry", "p2_entry"]].fillna("unknown")
...
df_data[["p1_ht", "p2_ht"]] = SimpleImputer(
    missing_values=np.nan, strategy='mean').fit_transform(df_data[["p1_ht", "p2_ht"]])
...
mapping = np.array([(c, c.replace("p2", "p1"))
                   for c in df_data_mirror.columns if c.startswith("p2")])
...
def match_statistics(player_id: str, until_date: date, Δ_days: int, min_match: int = 5) -> float:
    ...
    """
    df_player = df_data[df_data["p1_id"] == player_id].copy(
    )  # Make a copy to avoid modifying the original data

    df_player["tourney_date"] = pd.to_datetime(
        df_player["tourney_date"])  # Convert to datetime format
...
Δ_days = 365
p1_stats, p2_stats = np.full(
    len(df_data), np.nan), np.full(len(df_data), np.nan)
for index in range(len(df_data)):
    p1_id, p2_id, match_date = df_data["p1_id"].iloc[index], df_data[
        "p2_id"].iloc[index], df_data["tourney_date"].iloc[index]
    p1_stats[index], p2_stats[index] = match_statistics(
        p1_id, match_date, Δ_days), match_statistics(p2_id, match_date, Δ_days)
...
df_data["Δ_rank_points"] = df_data["p1_rank_points"] - \
    df_data["p2_rank_points"]

y = df_data[outcome]

# remove tournament and player ids
X = df_data.drop([outcome, "tourney_id", "p1_id", "p2_id"],
                 axis="columns").set_index("tourney_date")

# dummie coding of categorical variables
X = pd.get_dummies(X, columns=["tourney_level", "surface",
                   "round", "p1_entry", "p1_hand", "p2_entry", "p2_hand"])
...
print(
    f"training set size: {len(ytrain)}; validate set size: {len(yvalidate)}; test set size: {len(ytest)}")
...
model = RandomForestClassifier(
    n_estimators=1024, criterion="gini", min_samples_split=16, n_jobs=4, random_state=2022)
...
fig, axes = plt.subplots(1, 2, gridspec_kw={'width_ratios': [
                         2, 3]}, figsize=(10, 4), dpi=100)
...
importances = permutation_importance(
    model, Xvalidate, yvalidate, scoring="roc_auc", n_repeats=10, n_jobs=4, random_state=2022)
...
auc = roc_auc_score(yvalidate, model.predict_proba(
    Xvalidate[features_keep])[:, 1])
print(f"auc on validation data: {auc}")

features_keep = ["Δ_stat", "Δ_rank_points", "Δ_rank",
                 "p1_stat", "p2_stat", "p1_rank_points", "p2_rank_points"]

# concatenate train and validate data
df_sns = pd.concat((
    pd.concat((Xtrain[features_keep], Xvalidate[features_keep]),
...
df_sns[""] = ""
fig, axes = plt.subplots(nrows=2, ncols=int(
    np.ceil(len(features_keep)/2)), figsize=(12, 8), dpi=80)
axes = list(itertools.chain.from_iterable(axes))

for (index, xname) in enumerate(features_keep):
    ax = axes[index]
    sns.violinplot(data=df_sns, y=xname, x="", hue="p1_won",
                   split=True, orient="v", cut=0, inner="quartile", ax=ax)
...
# fit model
model.fit(pd.concat((Xtrain[features_keep], Xvalidate[features_keep]), axis=0), pd.concat(
    (ytrain, yvalidate), axis=0))
